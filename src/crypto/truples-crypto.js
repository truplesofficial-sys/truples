...
import { SesameEngine, DeviceRecord, DeviceStatus, MultiDeviceEncryptedPayload } from './truples-sesame.js';
...
/**
 * TRP-004: Canonical Handshake Transcript Binary Encoder.
 * Cryptographically binds domain, protocol version, role, identities, ephemerals, and salt.
 * Binary format:
 * [4-byte domainLen] [domain ASCII bytes]
 * [4-byte version]
 * [1-byte role: 0x01 initiator, 0x02 responder]
 * [4-byte initIdLen] [initiatorIdentityRaw]
 * [4-byte respIdLen] [responderIdentityRaw]
 * [4-byte initEphLen] [initiatorEphemeralRaw]
 * [4-byte respEphLen] [responderEphemeralRaw]
 * [4-byte saltLen] [saltBytes]
 */
export function canonicalEncodeHandshakeTranscript({
  domain = 'TRUPLES-HANDSHAKE-v2',
  version = 1,
  role = 'initiator',
  initiatorIdentityRaw,
  responderIdentityRaw,
  initiatorEphemeralRaw,
  responderEphemeralRaw,
  saltBytes
}) {
  const enc = new TextEncoder();
  const domainBytes = enc.encode(domain);

  const initId = typeof initiatorIdentityRaw === 'string' ? base64ToBytes(initiatorIdentityRaw) : initiatorIdentityRaw;
  const respId = typeof responderIdentityRaw === 'string' ? base64ToBytes(responderIdentityRaw) : responderIdentityRaw;
  const initEph = typeof initiatorEphemeralRaw === 'string' ? base64ToBytes(initiatorEphemeralRaw) : initiatorEphemeralRaw;
  const respEph = typeof responderEphemeralRaw === 'string' ? base64ToBytes(responderEphemeralRaw) : responderEphemeralRaw;
  const salt = typeof saltBytes === 'string' ? base64ToBytes(saltBytes) : (saltBytes || new Uint8Array(32));

  const totalLen = 4 + domainBytes.byteLength + 4 + 1 +
    4 + initId.byteLength +
    4 + respId.byteLength +
    4 + initEph.byteLength +
    4 + respEph.byteLength +
    4 + salt.byteLength;
  ...
  // Domain
  view.setUint32(offset, domainBytes.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, domainBytes.byteLength).set(domainBytes); offset += domainBytes.byteLength;

  // Version
  view.setUint32(offset, version, false); offset += 4;

  // Role
  view.setUint8(offset, role === 'initiator' ? 0x01 : 0x02); offset += 1;

  // Initiator Identity
  view.setUint32(offset, initId.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, initId.byteLength).set(initId); offset += initId.byteLength;

  // Responder Identity
  view.setUint32(offset, respId.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, respId.byteLength).set(respId); offset += respId.byteLength;

  // Initiator Ephemeral
  view.setUint32(offset, initEph.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, initEph.byteLength).set(initEph); offset += initEph.byteLength;

  // Responder Ephemeral
  view.setUint32(offset, respEph.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, respEph.byteLength).set(respEph); offset += respEph.byteLength;

  // Salt
  view.setUint32(offset, salt.byteLength, false); offset += 4;
  new Uint8Array(buffer, offset, salt.byteLength).set(salt); offset += salt.byteLength;
...
export class TruplesCryptoCore {
  /**
   * Static alias for canonical header encoding (TRP-010).
   */
  static canonicalEncodeHeader(header) {
    return canonicalEncodeHeader(header);
  }

  /**
   * Static alias for canonical transcript encoding (TRP-004).
   */
  static canonicalEncodeHandshakeTranscript(params) {
    return canonicalEncodeHandshakeTranscript(params);
  }
  /**
  ...
  /**
   * Executes a MITM-resistant Authenticated Key Exchange with strict Canonical Transcript Binding.
   */
  static async deriveAuthenticatedRootAndChainKeys(
    ...
    if (!transcriptParams) {
      throw new Error('Cryptographic Handshake Aborted: transcriptParams is required for canonical transcript binding.');
    }

    const dataToVerify = canonicalEncodeHandshakeTranscript(transcriptParams);
  ...
  /**
   * Legacy handshake method (raw public key signing without transcript binding).
   */
  static async deriveLegacyAuthenticatedRootAndChainKeys(
    localEcdhPrivateKey,
    remoteEcdhPublicKey,
    remoteEcdsaIdentityPublicKey,
    remoteSignatureBase64,
  ...
  /**
   * Encodes Double Ratchet header into canonical 113-byte binary AAD format.
   * Layout: [msgNum: 4B big-endian | prevLen: 4B big-endian | dhPubKey: 97B uncompressed 0x04 | magic: 4B | checksum: 4B]
   */
  static canonicalEncodeHeader(header) {
    const pubKeyBytes = base64ToBytes(header.dhPublicKey);
    if (pubKeyBytes.byteLength !== 97 || pubKeyBytes[0] !== 0x04) {
      throw new Error('Invalid P-384 public key encoding for AAD binding');
    }
    if (header.messageNumber > 0xFFFFFFFF || header.previousChainLength > 0xFFFFFFFF) {
      throw new Error('Integer overflow in message counters');
    }

    const aad = new Uint8Array(113);
    const view = new DataView(aad.buffer);
    view.setUint32(0, header.messageNumber, false);
    view.setUint32(4, header.previousChainLength, false);
    aad.set(pubKeyBytes, 8);
    view.setUint32(105, 0x54525550, false); // 'TRUP'
    view.setUint32(109, 0x4C455331, false); // 'LES1'
    return aad;
  }
  ...
  static async computeSafetyNumber(keyA, keyB) {
    ...
    // Canonical byte-wise lexicographical comparison (Zero Node.js Buffer dependency, 100% Browser & Mobile safe)
    let cmp = 0;
    const minLen = Math.min(rawA.length, rawB.length);
    for (let i = 0; i < minLen; i++) {
      if (rawA[i] !== rawB[i]) {
        cmp = rawA[i] < rawB[i] ? -1 : 1;
        break;
      }
    }
    if (cmp === 0 && rawA.length !== rawB.length) {
      cmp = rawA.length < rawB.length ? -1 : 1;
    }
...
export class DoubleRatchetSession {
  ...
  /**
   * Explicitly triggers an outbound asymmetric DH Ratchet turn.
   */
  async initiateDhRatchetTurn() {
