...
const { TruplesCryptoCore, DoubleRatchetSession, IdentityStore, PersistentStorageEnclave, canonicalEncodeHeader, canonicalEncodeHandshakeTranscript } = require('../src/crypto/truples-crypto');
...
async function runCryptographicTestSuite() {
  ...
  const bobIdentRaw = await globalThis.crypto.subtle.exportKey('raw', bobIdentity.publicKey);
  const aliceIdentRaw = await globalThis.crypto.subtle.exportKey('raw', aliceIdentity.publicKey);

  const dynamicSalt = new Uint8Array(32);
  globalThis.crypto.getRandomValues(dynamicSalt);

  const bobTranscriptParams = {
    domain: 'TRUPLES-HANDSHAKE-v2',
    version: 1,
    role: 'responder',
    initiatorIdentityRaw: new Uint8Array(aliceIdentRaw),
    responderIdentityRaw: new Uint8Array(bobIdentRaw),
    initiatorEphemeralRaw: new Uint8Array(aliceEcdhRaw),
    responderEphemeralRaw: new Uint8Array(bobEcdhRaw),
    saltBytes: dynamicSalt
  };
  const bobTranscriptBytes = canonicalEncodeHandshakeTranscript(bobTranscriptParams);
  const bobSignedTranscript = await TruplesCryptoCore.signPayload(bobTranscriptBytes, bobIdentity.privateKey);

  const aliceTranscriptParams = {
    domain: 'TRUPLES-HANDSHAKE-v2',
    version: 1,
    role: 'initiator',
    initiatorIdentityRaw: new Uint8Array(aliceIdentRaw),
  ...
  const aliceTranscriptBytes = canonicalEncodeHandshakeTranscript(aliceTranscriptParams);
  const aliceSignedTranscript = await TruplesCryptoCore.signPayload(aliceTranscriptBytes, aliceIdentity.privateKey);
  ...
  const bobKeys = await TruplesCryptoCore.deriveAuthenticatedRootAndChainKeys(
    ...
    aliceSignedTranscript,
    dynamicSalt,
    'responder',
    aliceTranscriptParams
  );
  ...
  // Verify missing transcriptParams throws error in strict mode
  let missingTranscriptBlocked = false;
  try {
    ...
    missingTranscriptBlocked = true;
  }
  assert(missingTranscriptBlocked, 'Handshake without transcriptParams must be rejected in strict mode');

  const eveIdentity = await TruplesCryptoCore.generateECDSAKeypair();
  const eveFakeSignature = await TruplesCryptoCore.signPayload(bobTranscriptBytes, eveIdentity.privateKey);
  ...
  console.log('   ✅ Passed: Verified Strict Canonical Transcript Binding and blocked MITM spoofing.\n');
